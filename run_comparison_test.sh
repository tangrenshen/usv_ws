...
rm -f comparison_results.txt

echo "[1/2] 编译并运行历史代码..."
cp src/usv_perception/src/main.cpp src/usv_perception/src/main.cpp.current
cp src/usv_perception/src/main.cpp.bak src/usv_perception/src/main.cpp
colcon build --symlink-install > /dev/null 2>&1

run_test "历史代码" 3

echo "[2/2] 编译并运行当前代码..."
cp src/usv_perception/src/main.cpp.current src/usv_perception/src/main.cpp
rm src/usv_perception/src/main.cpp.current
colcon build --symlink-install > /dev/null 2>&1
...
HIST_RECALL=$(grep "历史代码_recall_median" comparison_results.txt | sed 's/.*=//')
CURR_RECALL=$(grep "当前代码_recall_median" comparison_results.txt | sed 's/.*=//')
HIST_FPR=$(grep "历史代码_fpr_median" comparison_results.txt | sed 's/.*=//')
CURR_FPR=$(grep "当前代码_fpr_median" comparison_results.txt | sed 's/.*=//')
