...
Cross-references [BlockRawPts] (per-pillar, per-frame raw point count within
...
PILLAR_RE = re.compile(
    r"\[BlockRawPts\] t=([\d.]+) block=(\d+) bx=([-\d.eE]+) by=([-\d.eE]+) "
    r"n1m=(\d+) n3m=(\d+)"
)
...
print(f"parsed {len(pillar_rows)} BlockRawPts rows")
