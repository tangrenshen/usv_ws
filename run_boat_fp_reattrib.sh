...
echo "[3/5] 启动感知节点(boat_tracker_enabled:=false)..."
source install/setup.bash
nohup ros2 run usv_perception perception_node --ros-args -p use_sim_time:=true -p boat_tracker_enabled:=false > perception_log.log 2>&1 &
PERCEPTION_PID=$!
sleep 3
