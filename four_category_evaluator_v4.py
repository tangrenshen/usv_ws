#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""四类目标评估 v4 —— v3 + 检测流时间映射修正（见 main 中 v4 注释），口径不变。
...
def estimate_det_offset(path):
    """检测挂钟 − 真值挂钟 的常数偏移，按 jsonl 接收顺序估计（见 main 中注释）。"""
    import json
    g_last = prev = None
    diffs, per = [], []
    with open(path) as f:
        for line in f:
            ...
            if r.get("type") == "gt":
                if prev is not None:
                    per.append(r["stamp"] - prev)
                prev = g_last = r["stamp"]
            elif r.get("type") == "det" and g_last is not None:
                diffs.append(r["stamp"] - g_last)
    if not diffs or not per:
        raise SystemExit("无法估计检测时间偏移：日志中缺少 gt 或 det 记录")
    return statistics.median(diffs) - statistics.median(per) / 2
...
def main():
    ...
    ap.add_argument("--common-clock", action="store_true",
                    help="日志内所有 stamp 已在同一时钟（如录制器接收时刻），不做任何映射")
    args = ap.parse_args()
    ...
    # v4 修正（2026-09-11）：检测流**不能**再按自身首末时刻单独线性映射。
    # 真值 stamp 是录制时的挂钟、检测 stamp 是回放时的挂钟，回放为 1 倍速，
    # 二者之间只差一个常数偏移、没有比例。v3 对两条流各自做首末对齐，而检测流
    # 比真值流晚开始、早结束（节点启动/首帧预热），于是被拉伸（run1 比例
    # 0.665 vs 真值 0.608），中段错位数秒——对静止目标几乎无影响，对以约
    # 1 m/s 运动的船只则使大量真实命中落到 2 m 容差之外。
    # 偏移由 jsonl 的**写入顺序**（即接收顺序）估计：每条检测写入时，最近一条
    # 已写入的真值 stamp 为 g_last，d - g_last 落在 [偏移, 偏移+真值周期]，
    # 取中位数减半个真值周期。检测随后经与真值相同的映射进入 odom 时间轴。
    # 自校验：对船只真值再加 Δ 扫描，命中应在 Δ=0 取峰（run1 实测峰在 0，
    # ±2s 即从 78.5% 恶化到 93%+；v3 的峰在 −3~−4s）。
    if args.common_clock:
        det_off, gt_map = 0.0, None
    else:
        det_off = estimate_det_offset(args.log_path)
        gt_map  = remap([gk for _, gk, _ in CATS])
    if gt_map:
        a, _, scale = gt_map
        for _, _, dk in CATS:
            for r in records[dk]:
                r["stamp"] = o0 + (r["stamp"] - det_off - a) * scale
    det_map = None
    offset_source = "common clock (identity)" if args.common_clock else f"gt: linear map to odom timeline; det: constant wall-clock offset {det_off:.3f}s then same map"
    offset = 0.0
